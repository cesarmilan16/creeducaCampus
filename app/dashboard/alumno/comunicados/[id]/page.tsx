...
export default async function ComunicadoAlumnoDetailPage({
  params,
}: {
  ...
  const { data, error } = await supabase
    .from('comunicados')
    .select('id, titulo, cuerpo, created_at, clase_id, clases(nombre), usuarios!comunicados_autor_id_fkey(nombre)')
    .eq('id', id)
    .single()
  ...
  const autor = Array.isArray(comunicado.usuarios) ? comunicado.usuarios[0] : comunicado.usuarios

  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <Link
        href="/dashboard/alumno/comunicados"
      ...
      <div className="rounded-2xl border border-[#E5E0D9] bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 border-t border-gray-100 pt-4 text-xs text-gray-400">
          {autor?.nombre && <span className="font-medium text-gray-500">Por {autor.nombre} · </span>}
